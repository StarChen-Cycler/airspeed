"""ROS2 node shell — thin bridge: config → subscriptions → adapters → writer."""
...
from core.runtime.stream_tracker import StreamTracker
...
class PlatformCollectionNode(Node):
    """ROS2 shell over the simplified data collection pipeline."""
    ...
    def __init__(
        ...
        self.get_logger().info(
            f"node initialized: {len(self._adapters)} streams, "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Platform collection node")
